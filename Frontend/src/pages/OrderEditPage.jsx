...
import { PenSquare, Minus, Plus, X, Search } from 'lucide-react'
...
export default function OrderEditPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      apiFetch('/items'),
      order ? Promise.resolve(order) : apiFetch('/orders').then((list) => list.find((o) => String(o.id) === id)),
    ])
  ...
  function stockFor(itemId) {
    return items.find((i) => i.id === itemId)?.stock_quantity ?? 0
  }
  ...
  function changeQty(itemId, delta) {
    setCart((prev) => {
      ...
      if (newQty > stockFor(itemId)) return prev
  ...
  async function handleSave() {
    ...
    if (cartLines.length === 0) {
      setError('Order must have at least one item — remove the order entirely from Order History instead if it should be cancelled.')
      return
    }
    ...
    try {
      ...
      const updated = await apiFetch(`/orders/${id}/items`, {
        method: 'PATCH',
      ...
      const message = err.data?.errors?.items?.[0] || err.data?.message || 'Failed to update order.'
      setError(message)
    } finally {
      setSubmitting(false)
    }
  ...
  if (!order) return <p style={{ color: 'var(--color-danger)' }}>{error || 'Order not found.'}</p>

  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 4 }}>
        <PenSquare size={22} color="var(--color-primary)" />
        <h1 style={{ fontSize: 22 }}>Edit Order #{order.id}</h1>
      </div>
      <p style={{ color: 'var(--color-text-secondary)', fontSize: 14, marginBottom: 20 }}>
        {order.customer?.name} ({order.customer?.phone}) — change items while the order is still being picked.
      </p>
          ...
          <div style={{ flex: '2 1 400px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(170px, 1fr))', gap: 14 }}>
              ...
              {visibleItems.length === 0 && (
                <p style={{ color: 'var(--color-text-muted)' }}>No items match "{searchQuery}".</p>
              )}
            </div>
          </div>

          <div className="card" style={{ flex: '1 1 300px', padding: 20, alignSelf: 'flex-start' }}>
            <h2 style={{ fontSize: 16, marginBottom: 14 }}>Updated Order</h2>

            {cartLines.length === 0 && <p style={{ color: 'var(--color-text-muted)' }}>No items selected.</p>}
            ...
            {successMessage && <p style={{ color: 'var(--color-success)', fontSize: 14, marginTop: 10 }}>{successMessage}</p>}
